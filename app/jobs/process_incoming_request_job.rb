...
class ProcessIncomingRequestJob < ApplicationJob
  ...
  def perform(*args)
    ...
    requests.each do |r|
      ...
      expense = Expense.create(source_type: r.data['source_type'], source_id: r.data['source_id'], sum: r.data['sum'], payment_date: r.data['payment_date'], description: r.data['description'], author_id: r.data['author_id'])
