...
class ProcessStatusChangedReportsJobTest < ActiveJob::TestCase
  test "should process status changed report" do
    expense = Expense.create(sum: 1000.00, author: users(:user2), source: bank_accounts(:first), status: 'agreed')
    ExpenseApiUser.create(expense: expense, api_user: api_users(:active))
    StatusChangedReport.create(expense: expense, status: expense.status_before_type_cast, responsible: users(:user1))
